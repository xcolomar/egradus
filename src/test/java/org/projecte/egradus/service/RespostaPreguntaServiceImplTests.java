...
public class RespostaPreguntaServiceImplTests {
	 ...
	 * @return
	 */
	private Alumne nouAlumne(String nom, String primerLlinatge, String correu, String alies, String clau, Date dataAlta,
	...
	private Pregunta novaPreguntaREC(Professor professor) {
		Pregunta pregunta = UtilTest.inserirPregunta(professor, PRE_ENUNCIAT, PRE_DIFICULTAT_TEO, PRE_DIFICULTAT_PRA,
				PRE_RAONAR_RESPOSTA, PRE_DATA_ALTA, PRE_TIPUS_REC, null, Pregunta.ESTAT_PUBLIC);
	...
	public void testEnviaPregunta() {
		...
		Alumne alumne2 = nouAlumne(ALU2_NOM, ALU2_PRIMER_LLINATGE, ALU2_CORREU, ALU2_ALIES, ALU2_CLAU,
				UtilTest.getDate(ALU2_DATA_ALTA), assignatura);
	...
	public void testEnviaPreguntaAMoltsAlumnes() {
		...
		Alumne alumne2 = nouAlumne(ALU2_NOM, ALU2_PRIMER_LLINATGE, ALU2_CORREU, ALU2_ALIES, ALU2_CLAU,
				UtilTest.getDate(ALU2_DATA_ALTA), assignatura);
		Alumne alumne3 = nouAlumne(ALU3_NOM, ALU3_PRIMER_LLINATGE, ALU3_CORREU, ALU3_ALIES, ALU3_CLAU,
				UtilTest.getDate(ALU3_DATA_ALTA), assignatura);
		Alumne alumne4 = nouAlumne(ALU4_NOM, ALU4_PRIMER_LLINATGE, ALU4_CORREU, ALU4_ALIES, ALU4_CLAU,
				UtilTest.getDate(ALU4_DATA_ALTA), assignatura);
		Alumne alumne5 = nouAlumne(ALU5_NOM, ALU5_PRIMER_LLINATGE, ALU5_CORREU, ALU5_ALIES, ALU5_CLAU,
				UtilTest.getDate(ALU5_DATA_ALTA), assignatura);
		alumneService.insereixAlumne(alumne2);
	...
	public void testGetRespostaPregunta() {
		...
		List<Alumne> alumnes = new ArrayList<Alumne>();
		alumnes.add(alumne);
		List<RespostaPregunta> llistaRp = respostaPreguntaService.enviaPregunta(pregunta.getCodi(), professor, alumnes,
				Boolean.FALSE);
	...
	public void testIniciaContestacioPreguntaFals() {
		...
		// com que no feim una crida a 'iniciarContestacioPregunta(...);', la pregunta
		// no s'ha
	...
	public void testIniciaContestacioPreguntaVertader() {
		...
		// com que s'ha iniciat la contestació de la pregunta, ja existirà data d'inici
		// de contestació
	...
	public void testGetOpcionsContestadesBuit() {
		...
		// com que no s'ha finalitzat la contestació de la pregunta, no hi poden haver
		// opcions marcades.
		// De totes maneres, iniciarContestacioPregunta crea un array buit, per tant la
		// comprovació no és
	...
	public void testGetOpcionsContestades() {
		...
		// com que s'ha finalitzat la contestació de la pregunta, hi poden haver opcions
		// marcades.
	...
	public void testFinalitzaContestacioPreguntaSenseOpcionsVertader() {
		...
		// Obtenim el num total de contestacions en aquest moment per a fer
		// comprovacions després
		int numTotalActualitzacionsNotaMitja = pregunta.getNumTotalActualitzacionsNotaMitja();

		// finalitzam la contestació de la pregunta
		rp = respostaPreguntaService.finalitzarContestacioPregunta(rp, RP_TEXT_RR, RP_TEXT_REC,
				new ArrayList<OpcioResposta>());
		pregunta = rp.getPregunta();

		// com que s'ha iniciat i finalitzat la contestació de la pregunta, ja existirà
		// data de fi de contestació
	...
	public void testFinalitzaContestacioPreguntaFals() {
		...
		// com que no feim una crida a 'finalitzarContestacioPregunta(...);', la
		// pregunta no s'ha
		// acabat de contestar:
		assertNotNull(rp.getDataContestacioInici());
	...
	public void testFinalitzaContestacioPreguntaCorreccioAutomatica() {
		...
		// abans de contestar la pregunta, agafam l'estat actual de la dificultat
		// pràctica i del total
		...
		// comprovam que no hi ha cap contestació i que la dificultat pràctica encara no
		// està definida
		assertNull(dp);
		...
		// En aquest punt la nota mitja i la dificultat pràctica encara no s'han
		// assignat
		...
		// comprovam que la nota mitja i la dificultat pràctica ja està definida i que
		// el num de
	...
	public void testGetPreguntesContestades() {
		...
		// finalitzam la contestació de la pregunta
		respostaPreguntaService.finalitzarContestacioPregunta(rp, RP_TEXT_RR, RP_TEXT_REC,
				new ArrayList<OpcioResposta>());
	...
	public void testGetPreguntesPerCorregirPreguntaREC() {
		...
		List<Alumne> alumnes = new ArrayList<Alumne>();
		alumnes.add(alumne);
		List<RespostaPregunta> llistaRp = respostaPreguntaService.enviaPregunta(preguntaREC.getCodi(), professor,
				alumnes, Boolean.FALSE);
		RespostaPregunta rp = llistaRp.get(0);
		...
		// comprovam que no hi ha cap contestació i que la dificultat pràctica i la nota
		// mitja encara no estan definides
		assertNull(pregunta.getDificultatPractica());
		assertNull(pregunta.getNotaMitja());
		...
		// comprovam que efectivament el professor ja té una pregunta pendent de
		// corregir, degut a que
		...
		// comprovam que la dificultat pràctica i la nota mitja segueixen sense estar
		// definides
		assertNull(pregunta.getDificultatPractica());
		assertNull(pregunta.getNotaMitja());

		// el temps de resposta mig s'han actualitzat (ja que la pregunta ha sigut
		// contestada)
		// però la nota mitja no (ja que la pregunta encara no ha sigut corregida, degut
		// a que és REC i requereix
	...
	public void testGetPreguntesPerCorregirPreguntaNoREC() {
		...
		// comprovam que, així i tot, el professor segueix sense tenir cap pregunta per
		// corregir,
	...
	public void testCorreccioPreguntaREC() {
		...
		List<Alumne> alumnes = new ArrayList<Alumne>();
		alumnes.add(alumne);
		List<RespostaPregunta> llistaRp = respostaPreguntaService.enviaPregunta(p.getCodi(), professor, alumnes,
				Boolean.FALSE);
		RespostaPregunta rp = llistaRp.get(0);
		...
		// comprovam que la dificultat pràctica i la nota mitja encara no estan
		// definides
		assertNull(pregunta.getDificultatPractica());
		assertNull(pregunta.getNotaMitja());
	...
	public void testIniciaContestacioPregunta() {
		...
		// Obtenim la pregunta per a poder verificar que és la que hem assignat al
		// principi
		...
		// Comprovam que no hi ha cap opció marcada (És a dir, existeix una llista,
		// sense cap opció)
	...
	public void testFinalitzaContestacioPregunta() {
		...
		// Abans de finalitzar la contestació, ens asseguram que no hi ha definida una
		// data de finalització
		// de la contestació, que el flag contestada està a fals, i que no s'ha inserit
		// cap text de RR ni que
		...
		// comprovam que tampoc s'ha assignat cap nota, ni s'ha actualitzat el camp de
		// dificultat pràctica
		// de la pregunta, ni el total de contestacions, ni es mostra com a corregida ni
		// existeix data de correcció
		...
		// Comprovam que, donat que la pregunta és de tipus VOF, s'ha pogut corregir
		// automàticament i, per tant,
		// té nota, la dificultat pràctica s'ha actualitzat, el num de contestacions
		// també, apareix com a
	...
	public void testFinalitzaContestacioPreguntaREC() {
		...
		// de la contestació, que el flag contestada està a fals, i que no s'ha inserit
		// cap text de RR ni que
		// s'ha seleccionat cap opció, ni hi ha cap text de resposta (en cas que sigui
		// una pregunta REC)
		assertNull(rp.getDataContestacioFi());
		assertFalse(rp.getContestada());
		assertNull(rp.getTextRaonarResposta());
		...
		// comprovam que tampoc s'ha assignat cap nota, ni s'ha actualitzat el camp de
		// dificultat pràctica
		// de la pregunta, ni la nota mitja, ni el total de contestacions, ni el temps
		// de resposta mig, ni
		...
		// Finalitzam la contestació d'aquesta RP
		rp = respostaPreguntaService.finalitzaContestacio(rp, CNT_TEXT_RR, CNT_TEXT_RESPOSTA,
				new ArrayList<OpcioResposta>());
		...
		// Comprovam que, donat que la pregunta és de tipus REC, NO s'ha pogut corregir
		// automàticament i, per tant,
		// no té nota, ni s'ha actualitzat la dificultat pràctica, ni la nota mitja, no
		// apareix com a corregida i no
