...
public class RespostaQuestionariServiceImplTests {
	...
	public void initialize() {
		...
		personaAlumne3 = UtilTest.inserirPersona(ALU3_NOM, ALU3_PRIMER_LLINATGE, ALU3_CORREU, ALU3_ALIES, ALU3_CLAU, UtilTest.getDate(ALU3_DATA_ALTA));
		personaAlumne4 = UtilTest.inserirPersona(ALU4_NOM, ALU4_PRIMER_LLINATGE, ALU4_CORREU, ALU4_ALIES, ALU4_CLAU, UtilTest.getDate(ALU4_DATA_ALTA));
		personaAlumne5 = UtilTest.inserirPersona(ALU5_NOM, ALU5_PRIMER_LLINATGE, ALU5_CORREU, ALU5_ALIES, ALU5_CLAU, UtilTest.getDate(ALU5_DATA_ALTA));
		...
		// Inserim el rol d'alumne per a les 5 persones alumne 'personaAlumneX' a l'assignatura 'assignatura'
	...
	public void testEnviaQuestionari() {
		...
		// dels alumnes que han rebut el qüestionari, hem de comprovar que aquestes resposta-questionaris
	...
	public void testIniciaContestacioQuestionariFals() {
		...
		// com que no feim una crida a 'iniciarContestacioQuestionari(...);', el qüestionari no s'ha
	...
	public void testIniciaContestacioQuestionariVertader() {
		...
		// com que s'ha iniciat la contestació del qüestionari, ja existirà data d'inici de contestació ...
		assertNotNull(rq.getDataContestacioInici());
		assertNull(rq.getDataContestacioFi());
		assertFalse(rq.getContestat());
		
		// Comprovam que les dates d'inici de contestació s'han assignat també a les resposta-preguntes
		// però encara no hi ha cap opció marcada (ja que no hem començat a marcar opcions)
	...
	public void testFinalitzaContestacioQuestionariFals() {
		...
		// com que no feim una crida a 'finalitzarContestacioQuestionari(...)', el qüestionari no
	...
	public void testFinalitzaContestacioQuestionariAmbCorreccioAutomatica() {
		...
		// comprovam que no l'ha contestat ningú, i que no s'han definit encara temps de resposta mig,
		...
		// com que s'ha finalitzat la contestació del qüestionari, ja existirà data de fi de contestació
		assertNotNull(rq.getDataContestacioInici());
		assertNotNull(rq.getDataContestacioFi());
		assertTrue(rq.getContestat());
		
		// comprovam que s'han contestat les resposta-preguntes de la resposta-questionari
		for (RespostaPregunta rp : rq.getRespostaPreguntes()) {
			assertNotNull(rp.getDataContestacioInici());
			...
			// comprovam, a més, que s'han pogut corregir ja que no hi havia cap pregunta REC
			assertTrue(rp.getCorregida());
			assertNotNull(rp.getNota());
			
			// comprovam que s'ha actualitzat el num total de contestacions, la dificultat pràctica,
			// la nota mitja i el temps de resposta mig la pregunta de cada resposta-pregunta
			Pregunta pregunta = rp.getPregunta();
			assertTrue(pregunta.getNumTotalActualitzacionsNotaMitja() > 0);
			assertNull(pregunta.getTempsRespostaMig()); // el temps de resposta mig NO s'actualitza quan les RP venen de RQs
			assertNotNull(pregunta.getDificultatPractica());
			assertNotNull(pregunta.getNotaMitja());
		}
		
		// comprovam que el qüestionari s'ha corregit (ja que no hi havia cap pregunta REC) i 
		// que la nota de la resposta-questionari és la esperada (tenint en compte que la 1º 
		...
		// comprovam que la dificultat pràctica ja està definida i que el num de contestacions s'ha incrementat en 1
	...
	public void testFinalitzaContestacioQuestionariSenseCorreccioAutomatica() {
		...
		// abans de contestar el qüestionari, comprovam que no hi ha cap contestació i que la dificultat pràctica
		...
		// si la resposta-questionari s'ha contestat, s'ha d'haver actualitzat el temps de 
		...
		for (RespostaPregunta rp : rq.getRespostaPreguntes()) {
			...
			assertTrue(rp.getContestada());
			assertNotNull(rp.getDataContestacioFi());
			assertNull(pregunta.getTempsRespostaMig()); // les resposta-preguntes de la resposta-questionari no tenen temps assignat
			...
			if (pregunta.getTipus().equals(Pregunta.TIPUS_REC)) {
				...
				// NO s'han d'haver actualitzat la dificultat pràctica i la nota mitja de la pregunta
				...
				// s'han d'haver actualitzat la dificultat pràctica i la nota mitja de la pregunta
				assertNotNull(pregunta.getDificultatPractica());
				assertNotNull(pregunta.getNotaMitja());
			}
		}
		
		// comprovam que el qüestionari NO s'ha corregit (ja que hi havia una pregunta REC) i,
		...
		// Si la resposta-questionari no s'ha corregit, no s'han d'haver actualitzat la dificultat
	...
	public void testGetQuestionarisPerCorregir() {
		...
		OpcioResposta orPEs1 = new OpcioResposta();
		orPEs1.setRespostaPregunta(rpEs1);
		orPEs1.setOpcio(preguntaService.getOpcio(pEs1.getOpcions().get(1).getCodi())); // seleccionam la 2º opció de la pregunta ES1
		List<OpcioResposta> llistaOrPEs1 = new ArrayList<OpcioResposta>();
		llistaOrPEs1.add(orPEs1);
		rpEs1.setTextRaonarResposta("Raonament de la resposta a la pregunta Es1");
		rpEs1.setOpcionsMarcades(llistaOrPEs1);
		...
		OpcioResposta orPVof = new OpcioResposta();
		orPVof.setRespostaPregunta(rpVof);
		orPVof.setOpcio(preguntaService.getOpcio(pVof.getOpcions().get(1).getCodi())); // seleccionam la 2º opció de la pregunta VOF
		List<OpcioResposta> llistaOrPVof = new ArrayList<OpcioResposta>();
		llistaOrPVof.add(orPVof);
		rpVof.setTextRaonarResposta("Raonament de la resposta a la pregunta Vof");
		rpVof.setOpcionsMarcades(llistaOrPVof);
		...
		// En aquest punt, el professor té un qüestionari pendent de corregir, però no en
		// té cap de corregit
	...
	public void testGetQuestionarisPerCorregirFals() {
		...
		if (PRE1_RAONAR_RESPOSTA || PRE2_RAONAR_RESPOSTA) assertNotNull(respostaQuestionariService.getQuestionarisPerCorregir(professor));
		else assertNull(respostaQuestionariService.getQuestionarisPerCorregir(professor));
		
		assertNull(respostaQuestionariService.getQuestionarisCorregits(professor));
	}
	
	/**
	 * Verifica l'inici i el fi de correcció del qüestionari, així com
	 *  el get questionaris corregits
	 */
//	@Test
